...
use Core\Security\Authentication\Application\Provider\ProviderUpdateACLInterface;
...
final class OpenId implements ProviderInterface, ProviderUpdateACLInterface
{
    use LoggerTrait;
    ...
    {
        $user = $this->provider->getUser();
        if ($this->isAutoImportSupported() === true && $user === null) {
            $this->info("Start auto import");
            $this->provider->createUser();
            $this->info("User imported: " . $user->getName());
        }
